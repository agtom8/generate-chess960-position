...
def main():
    ...
    positions=()                                                                # tuple of all valid positions
    pos_cnt = 0
    ...
    # start a loop until all positions found
    while pos_cnt < 960:
        ...
        pos = ''
        for char in cols:
            pos = pos + position[char+str(rows)]+char+str(rows) + ' '
        pos = pos[0:len(pos)-1]
        if pos not in positions:
            positions = positions + (pos,)
            pos_cnt += 1
            sys.stdout.write("{:<4}{:<2}{:<31}\n".format(str(pos_cnt),':',pos))
    sys.stdout.write('\n')
    end_time = time.time()
